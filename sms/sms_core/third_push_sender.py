...
def send_channel(channel: str, message: str, settings: dict, user_agent="Air724UG-SMS", port: str = ""):
    if channel == "custom_post":
        ...
        return api_ok(channel, http_ok, code, body)
    ...
    if channel == "pushdeer":
        ...
        http_ok, code, body = http_request(url, "POST", {"Content-Type": "application/x-www-form-urlencoded"}, data, user_agent=user_agent)
        return api_ok(channel, http_ok, code, body)
    ...
    if channel == "dingtalk":
        ...
        http_ok, code, body = http_request(url, "POST", {"Content-Type": "application/json; charset=utf-8"}, data, user_agent=user_agent)
        return api_ok(channel, http_ok, code, body)
