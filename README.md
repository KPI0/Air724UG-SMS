...
- **信息保密设计**
  ...
  - 默认不上传服务器、不经过第三方平台，适用于内网部署和本地值守场景
  - 适用于涉密环境或内网部署
- **默认本地运行**
  - 未启用云控和第三方推送时，短信接收、显示、播报、来电处理等核心功能可离线运行
  - 启用云控后，软件会按用户配置连接 WebSocket 服务
  - 启用第三方推送后，软件会按用户配置请求钉钉、飞书、企业微信、Pushover、Gotify、SMTP 等接口
- **来电防骚扰**
