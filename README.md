...
- **外部连接与数据范围**
  ...
  - 启用第三方推送后，短信或来电数据会按用户配置发送至钉钉、飞书、企业微信、WxPusher、Pushover、Gotify、SMTP（SSL/STARTTLS）等外部服务
...
- **通话录音**
  - 接听来电后按固件配置开始录音；超过 50 秒时由固件自动分段，客户端按录音 ID 保存为一条完整 AMR 录音
  - 录音保存在客户端 `recordings/` 目录，并在云控授权恢复后自动补传控制台
  - 录音传输中、上传失败和已保存状态会在控制台对应电话事件中显示
- **主动呼叫支持**
  - 快捷拨打外部电话，支持国际前缀
